...
class GigRepository(
    ...
    private val httpClient: OkHttpClient = OkHttpClient.Builder()
        ...
        .followRedirects(false) // Handle redirects manually to preserve User-Agent and headers
    ...
    suspend fun fetchGigs(
        ...
        val builder = java.lang.StringBuilder("https://www.craigslist.org/search/area/$cleanPrefix")
        ...
        val initialUrl = if (params.isNotEmpty()) {
                ...
                httpClient.newCall(request).execute().use { response ->
                    ...
                    if (code in 300..399) {
                        ...
                        if (!location.isNullOrBlank()) {
                            ...
                            throw IOException("HTTP redirect $code with empty Location header for URL: $currentUrl")
                        }
                    } else {
                        if (!response.isSuccessful) {
                            throw IOException("HTTP error response: $code for URL: $currentUrl")
                        }
                        bodyString = response.body?.string() ?: ""
                        redirectCount = maxRedirects // End loop
                    }
                }
            }
    ...
    suspend fun fetchDescription(link: String): String = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (bodyHtml.isEmpty()) return@withContext ""
            
            val postingBodyStart = bodyHtml.indexOf("<section id=\"postingbody\">")
            if (postingBodyStart == -1) return@withContext ""
            
            val sub = bodyHtml.substring(postingBodyStart)
